...
| Both | build the skeleton + text with the bulk call, then add PDFs and later edits block by block or by reconcile, then republish what changed — rename the builder's `"Text"` html blocks *before* the first reconcile or it duplicates every one of them (`/iblai-api-studio-outline`) |
