...
description: Add and edit HTML (text) components in an Open edX unit on Studio — create the html block, write its HTML body and title in one update, reference uploaded images and files via /static/ paths, duplicate or delete it, and follow the authoring rules that make content look designed (cards, callouts, tables, tooltips via inline CSS) while leaving typography and page width to the LMS. Use when the user wants text, a reading, an explanation, a callout, a comparison table, an embedded video/iframe, or any rich content inside a unit. Session auth via studio.env; run the auth preflight first.
metadata:
  kind: api
---
