---
name: iblai-api-heygen
description: Drive HeyGen through ibl.ai's dedicated video API — list and clone voices, synthesize speech, create photo and digital-twin avatars, generate avatar videos and poll them, upload media assets, and run interactive streaming avatar sessions end to end. Covers the credit rules, the ownership gate, and the LiveKit handoff. Use when building avatar video generation, voice cloning, or a talking-avatar experience; see /iblai-api-external-service-proxy for the generic multi-provider proxy door.
metadata:
  kind: api
---
