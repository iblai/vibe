...
Studio **and** the LMS (the user side shares the same login), and stores it
...
| `LMS_APP_URL` | `https://lms.ibl.ai` (test: `https://lms.iblai.org`) — the LMS app | user URLs in reports |
| `STUDIO_ORG` | org key used as the default `org` when creating courses (set by hand; optional) | `/iblai-api-studio-course-create` |
...
Values are single-quoted; load with `set -a; . ./studio.env; set +a`.
...
2. **Set the default org** (optional, once): `STUDIO_ORG=<org key>` in
   `studio.env`. Course creation needs the org key of the organization the user
   authors in (the path segment after `/platform/` on os.ibl.ai, or `PLATFORM`
   in `iblai.env`). The script preserves this line on re-runs.

3. **Verify** (also what every other skill does when a call misbehaves):

   ```bash
   node .claude/skills/iblai-api-studio-auth/scripts/studio-login.mjs --check   # prints "studio: ok · lms: ok"; exit 2 if expired
   ```
   ...
   curl -s -o /dev/null -w '%{http_code}\n' -b "studio_session_id=$STUDIO_SESSION; csrftoken=$STUDIO_CSRF" \
     -H "Accept: application/json" "$STUDIO_URL/api/contentstore/v2/home/courses"   # 200 = signed in; 302 = expired
   ```
...
tab: `sessionid`. Write `studio.env` yourself with the keys above (quote the
values), `chmod 600`, add it to `.gitignore`, then run `--check`. Ask for
...
- **Expired session** looks like `302` to `/login/…` (Studio), `401` (LMS API),
  or `403` with a CSRF message. Re-run step 1; do not retry the call in a loop.
