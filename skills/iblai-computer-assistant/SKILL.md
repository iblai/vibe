...
description: Use when adding the "Computer Assistant" (let the on-device AI control the Mac — click, type, and operate apps) to a vibe Next.js + Tauri app. Documents the SDK contract (`systemControlProps`, the system-control hook, the install/stop/status commands + events, `MACOS_PERMISSIONS_COMMANDS`, the model-size gate `requiredSizeGb`, the Local-Models gate) and the macOS host implementation (a Homebrew-installed native control helper, the ollama-mcp-bridge that gives a local model MCP tools, the `tauri-plugin-macos-permissions` Accessibility flow, the non-sandboxed Developer ID build). Mention before wiring "system control", "computer use", or "let the AI use my computer" into a desktop build. macOS-only.
globs:
alwaysApply: false
---
...
operate the Mac — click buttons, type, fill forms, open apps — by giving it a
native **system-control helper** as an MCP tool server. The SDK surfaces it as a
plain-language card in **User Profile → Advanced** (next to "Local Models"),
labelled **"Computer Assistant"**.
...
through a local proxy that injects the helper's tools. So **Local Models must be
on and a tool-capable model selected** before any of this works.
...
This skill does **not** ship a reference implementation — the host commands and
the control helper are closed-source in the apps that have it. It defines the
**contract** and the integration gotchas. The SDK's exact export names differ
from the generic roles used here; **grep your installed `@iblai/web-containers`
`.d.ts`** for the literal hook / command / event / constant names — that is the
ground truth — and match them on the host.

## How the loop works

```
local chat (chat-stream command, tool_support=true)
   │  POST http://localhost:8000/api/chat        ← the MCP bridge, NOT :11434
   ▼
ollama-mcp-bridge  ──(injects the helper's MCP tools into the request)──► Ollama
   │                                              (tool-capable model, e.g. phi4-mini)
   │  model emits tool_calls (click, type, …)
   ▼
bridge spawns the helper's MCP server (stdio) ──► it drives the Mac via Accessibility
   │  tool results fed back; bridge loops until the model returns a final answer
   ▼
streamed back to the chat UI
```

Three moving parts the host must provide: the **helper installed** (Homebrew),
the **ollama-mcp-bridge running** with the helper as a server in its config, and
the **macOS Accessibility permission** granted. The card walks the user through
them.
...
2. **macOS only.** The control helper is a native macOS MCP server; Accessibility
   is a macOS TCC permission. Every host command no-ops / errors off-macOS.
...
### 1 — SDK surface: `systemControlProps` + the system-control hook
...
**self-wires** from the SDK's system-control hook (`use…`, exported from
`@iblai/iblai-js/web-containers`), so the host only passes `systemControlProps`
to configure things (e.g. the size gate). Grep your installed SDK for the exact
shape — it is the ground truth:
...
systemControlProps?: {
  ...
  state: InstallState                  // status/progress/message/logs/lastUpdated
  status: Status | null                // installed/running/installing/version
...
- **Config-only (recommended for just the gate).** The dropdown falls back to the
  hook per-field at runtime, so you can pass *only* `requiredSizeGb`. But the SDK
  types the object as fully-required, so cast:
  ```tsx
  systemControlProps={
  ...
  ```
- **Full props.** Call the hook yourself and pass every field (mirrors how
  `localLLMProps` is wired). Type-clean, but the dropdown *also* calls the hook
  internally, so it runs twice (double listeners) — prefer the cast unless you
  need to intercept the handlers.

The hook returns `{ isAvailable, state, status, accessibilityPermission, install,
stop, checkStatus, resetState, requestAccessibilityPermission }`. `isAvailable`
is `isTauriApp()`, so the card hides itself in a browser tab.

### 2 — Host commands (Rust)

Three `#[tauri::command]`s — **install**, **stop**, **status** — whose exact
names your SDK exports as a `…_TAURI_COMMANDS` map (grep it). Roles + shapes:

```
install   -> Result<String, String>      brew install; emits install-progress/log
stop      -> Result<(), String>          best-effort pkill of any live helper process
status    -> Result<Status, String>
```

```rust
struct Status { installed: bool, running: bool, installing: bool, version: Option<String> }
```

Events (your SDK's `…_TAURI_EVENTS` map), payloads the hook consumes:

```
<ns>:install-progress   { status: "installing"|"completed"|"error", percentage: f64, message }
<ns>:installation-log   { timestamp, level, message }       // shown in "technical details"
<ns>:status             Status
```

Two non-obvious rules baked into the host:

- **`install` = `brew install <your-control-helper-formula>` — and that is all.**
  Do **NOT** run the helper's own `setup` command. Its setup would prompt for
  Accessibility itself; the app owns that flow (step 3) so it can present the
  friendly card instead. (Setup also wires recipes/vision; the basic click/type
  tools work without it.)
- **`running` == `installed`, not a live process.** The helper is an *on-demand*
  stdio MCP server — the bridge spawns it only while a tool call is in flight, so
  there is no daemon to `pgrep`. If you key "running" on a live process the card
  sits on **"Starting…" forever**. Report `running: installed`.
...
process** — it reports whether *the app* (not the helper) is trusted. The helper,
spawned as a descendant of the app, inherits that trust.
...
- Configure the helper as a server in the user-editable `mcp-config.json`
  (`<app_data_dir>/mcp-config.json`), using the **absolute** binary path (the GUI
  app's PATH usually omits `/opt/homebrew/bin`):
  ```json
  { "mcpServers": { "computer-assistant": { "command": "/opt/homebrew/bin/<helper>", "args": ["mcp"] } } }
  ```
  Register the server **after** a successful install (so the bridge never starts a
  server that can't launch), then restart the bridge to pick it up.

### 5 — Chat routing (`tool_support`)

The SDK passes a `toolSupport` flag (from the selected model's catalog
`tool_support`) to the chat-stream command
(`stream(messages, model, generationId, toolSupport)`). The **host owns the
routing**:
...
Streaming gotcha: with tools, the bridge emits **one `done:true` per tool round**
(round 1 = the tool call, round 2 = the answer). Do **not** return on the first
`done` — only a `done` whose chunk carries **no `tool_calls`** is final. Log a
`running tool…` marker on each tool-round boundary.

### 6 — Gates

The card refuses to enable in two cases (both in the SDK's system-control tab):
...
- **Model too small.** `modelSupportsSystemControl(modelId, requiredSizeGb)` ⇒
  ...
  (default 12). It is a **prop now** — the old module-state setter was removed;
  setting it from a provider can't cross the SDK's `index`/`next` bundle split, so
...
- **Every host command must be granted in the capability ACL**, or the SDK's
  `invoke` fails with `… not allowed. Command not found`. App command permissions
  are **hand-maintained**, not auto-generated:
...
- **Don't run the helper's `setup`**, and report **`running == installed`** (see §2).

- **Resolve the helper / `brew` / `ollama-mcp-bridge` by absolute path.** A GUI
  app's PATH is minimal (`/usr/bin:/bin:…`, no Homebrew). Probe PATH then
...
user grants it — so the MAS build can show the prompt but the helper can't drive
