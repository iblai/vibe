...
Add the agent **Audit tab** -- a paginated audit log of agent-scoped
changes (create / update / delete) with humanized action summaries
(e.g. "Enabled tools on E2E Agent 123 (uuid)", "Updated forkable, LLM
model, ... on Settings for E2E Agent"), filterable by actor email,
...
### Switching the audited agent at runtime
...
| `mentor` | uuid | Filter to one agent |
...
- `resource_repr` ends with `(uuid)` for agent-scoped resources —
...
- The trailing UUID in `resource_repr` is the agent's `unique_id`,
  not its primary key — useful for cross-referencing with agent
