"""Default RBAC roles the ibl.ai platform seeds.

Use these as a starting point when authoring new roles via
`<RolesTab>` or `POST rbac/roles/` (see /iblai-vibe-agent-access → "Roles
CRUD" for the REST contract).

The exhaustive `data_actions` arrays exist to keep field-level reads
working for the existing UI; trim aggressively when authoring custom
roles that only need a subset of the surface.
"""
...
MENTOR_CREATORS = {
    "actions": ["Ibl.Mentor/Mentors/action"],
    "data_actions": [],
}
...
MENTOR_ANALYTICS_VIEWER = {
    ...
    + STUDENT["actions"],
    "data_actions": STUDENT["data_actions"],
}
...
# Allows a user to use the CRM system
CRM_USER = {
    "actions": [
        "Ibl.CRM/Persons/*",
