...
Roles + Policies — see `/iblai-vibe-rbac`); the CRM does not expose its own
...
- `<Admin>` (mounted via `<Account>` — see `/iblai-vibe-account`) →
...
- Or call `POST rbac/policies/` directly (see the REST contract
  under `/iblai-vibe-agent-access` → "Roles CRUD").
...
See `/iblai-vibe-rbac` for the full management-UI walkthrough and the
