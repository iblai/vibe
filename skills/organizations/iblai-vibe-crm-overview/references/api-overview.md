...
The at-a-glance contract that every CRM skill builds on. Pair with the
per-resource references in the `crm-contacts`, `crm-person-onboarding`,
`crm-pipeline`, `crm-activities`, and `crm-tags` skills.
