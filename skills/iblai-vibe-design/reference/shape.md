...
**Output**: A design brief ready to hand off to /iblai-vibe-design craft, or straight to /iblai-vibe-design for freeform implementation. When visual direction probes are used, the images are supporting artifacts, not the primary output.
...
Once confirmed, the brief is complete. The user can now hand it to /iblai-vibe-design, or use it to guide any other implementation approach. (If the user wants the full discovery-then-build flow in one step, they should use /iblai-vibe-design craft instead, which runs this command internally.)
