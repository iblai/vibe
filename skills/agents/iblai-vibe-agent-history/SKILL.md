...
description: Add the agent History tab (every user's conversations with this agent — filters, export, and a transcript panel with attachments, retrieved documents, tool calls, and per-turn metadata such as the model, Canvas and Proactive Prompt). Use when the user mentions conversation history, transcripts, chat logs, what people asked an agent, or exporting conversations. For the signed-in user's own history see /iblai-vibe-history; for the org-wide Transcripts tab see /iblai-vibe-analytics; for the REST endpoints see /iblai-api-agent-history.
globs:
alwaysApply: false
metadata:
  kind: ui
...
| **Retrieved Documents** | The chunks the answer cited, in the same dialog the chat opens, with source, snippet and relevance score |
