---
name: iblai-vibe-monetization-configure
...
- The caller has the `can_sell_items` RBAC permission on the Platform
  ...
  resolves this via `checkRbacPermission` — see `/iblai-vibe-rbac`.
- **Stripe Connect Express onboarding is complete** for the Platform.
  ...
  `/iblai-vibe-monetization-onboard` first and come back.
...
|---|---|---|
| Stripe Connect status card | Status badge, business info, connect / dashboard button | (covered by `/iblai-vibe-monetization-onboard`) |
...
The Account page (`/iblai-vibe-account`) is the host. It owns the sidebar
...
`https://auth.{ext}`) that `/iblai-vibe-auth` already configured. Do not
...
`/iblai-vibe-monetization-checkout` for the complete recipe.
...
- **Confusing `enable_monetization` with `show_paywall`.**
  ...
  `show_paywall` is the credit-system gate covered by `/iblai-vibe-credit`.
  They are not synonyms — see `/iblai-vibe-monetization` for the matrix.
...
- `/iblai-vibe-monetization` — Family index, schema validation, RTK Query overview, RBAC matrix.
- `/iblai-vibe-monetization-onboard` — **Prerequisite.** Stripe Connect Express onboarding, `is_ready_for_payments` gate, return URL using `?profileTab=monetization`.
- `/iblai-vibe-monetization-checkout` — What buyers see: `PaywallModal`, `useCheckAccessQuery`, checkout, guest checkout, public pricing (`useGetPublicItemPricingQuery`).
- `/iblai-vibe-monetization-subscription` — End-user `PurchasesTab` + cancel flow.
- `/iblai-vibe-monetization-analytics` — Revenue, subscribers, paywalls list, admin cancel tool.
- `/iblai-vibe-account` — Hosts the `MonetizationTab`; owns the sidebar and gating.
- `/iblai-vibe-auth` — Token wiring; every paywall and price call uses `Authorization: Token <token>`.
- `/iblai-vibe-rbac` — `can_sell_items` permission on `/platforms/{platformKey}/`.
