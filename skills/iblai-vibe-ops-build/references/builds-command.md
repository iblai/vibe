...
| `iconography <source>` | Generates every icon size from one image using ImageMagick (`convert`) — Tauri + MSIX + multi-res `.ico` + macOS `.icns`. Falls back to `tauri icon` when ImageMagick is missing. Requires `src-tauri/`. See [`../../iblai-vibe-iconography/SKILL.md`](../../iblai-vibe-iconography/SKILL.md). |
...
- Implementation internals: [`../../iblai-vibe-cli-maintenance/references/iblai-vibe-cli-builds.md`](../../iblai-vibe-cli-maintenance/references/iblai-vibe-cli-builds.md).
