---
name: iblai-credential
description: RBAC setup that lets an API token list and unmask integration credentials on the ibl.ai platform. Use when the user mentions integration credentials, unmasking sensitive credentials, or credential access for a token; see /iblai-rbac for the broader RBAC model.
globs:
alwaysApply: false
---
...
- [`/iblai-rbac`](../iblai-rbac/SKILL.md) — default roles, the action-definitions endpoint, and the SDK Roles + Policies components.
- [`/iblai-account`](../iblai-account/SKILL.md) — the Organization Settings surface where API keys, Roles, and Policies are managed.
