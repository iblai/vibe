...
The full `tenants` array is hydrated into `localStorage` by [/iblai-vibe-auth](../../../start/iblai-vibe-auth) after login; every entry carries both flags so any component can read them synchronously without a network round-trip.
...
- [/iblai-vibe-navbar](../../../start/iblai-vibe-navbar) — where `CreditBalance` is mounted
