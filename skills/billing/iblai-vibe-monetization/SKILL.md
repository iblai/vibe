...
| `/iblai-vibe-monetization-app-paywall` | Build app gate | Whole-app "pay to enter" gate on the organization's OWN Stripe key via the DM Stripe proxy paywall endpoints — server routes + `PaywallGate` + `/paywall` pricing page. No Connect, no commission |
...
Selling access to the whole app rather than items sits outside this sequence — it runs on the organization's own Stripe key, not Connect. See `/iblai-vibe-monetization-app-paywall`.
...
- `/iblai-vibe-monetization-app-paywall` — whole-app pay-to-enter gate on the organization's own Stripe key (DM Stripe proxy; no Connect, no commission)
