...
Four Platform-admin endpoints power custom monetization dashboards — the configured paywalls list, two flavors of subscriber listing (Platform-wide and per-item), and a revenue snapshot. All four are gated by `IsPlatformAdmin`; anonymous callers get 401, non-admin authenticated users get 403. Pair with [/iblai-vibe-monetization-analytics](../SKILL.md) for the UI playbook and [/iblai-vibe-monetization → references/schema-validation.md](../../iblai-vibe-monetization/references/schema-validation.md) for the live-schema check routine.
...
If the Stripe Connect account is not fully onboarded, `connected_account_amount` will not populate, and `sales_volume` will under-report. Confirm `is_ready_for_payments` via [/iblai-vibe-monetization-onboard](../../iblai-vibe-monetization-onboard/SKILL.md) before relying on revenue numbers.
...
Cancelling invalidates the caller's `accessCheck` cache, so they lose access at next page-load. For the user-side surface that wraps this in a list of all purchases, see [/iblai-vibe-monetization-subscription](../../iblai-vibe-monetization-subscription/SKILL.md).

## Related skills

- [/iblai-vibe-monetization](../../iblai-vibe-monetization/SKILL.md) — family index, RBAC matrix, item-type normalization.
- [/iblai-vibe-monetization-onboard](../../iblai-vibe-monetization-onboard/SKILL.md) — Stripe Connect status, commission display, payouts readiness.
- [/iblai-vibe-monetization-configure](../../iblai-vibe-monetization-configure/SKILL.md) — paywall + price CRUD covered by `useListPaywallsQuery`.
- [/iblai-vibe-monetization-subscription](../../iblai-vibe-monetization-subscription/SKILL.md) — user-side counterpart with portal_url vs immediate branch logic.
- [/iblai-vibe-rbac](../../../users/iblai-vibe-rbac/SKILL.md) — the `IsPlatformAdmin` gate.
