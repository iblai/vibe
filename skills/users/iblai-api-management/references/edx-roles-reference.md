...
| `course-creator` | Can create courses in **every** organization. Only assignable through the **CMS** (Studio) host - the LMS host rejects it with `400`. On a multi-organization deployment prefer `org-course-creator` instead (scoped to one org). |
...
- **`course-creator` is CMS-only** - the LMS host rejects it with `400`.
  Prefer `org-course-creator` on multi-organization deployments regardless of host.
