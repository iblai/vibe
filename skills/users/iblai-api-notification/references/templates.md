...
Alerts recipients when a user opens a human-support ticket from an AI agent
...
### Proactive user (AI) — `PROACTIVE_LEARNER_NOTIFICATION`

Emails AI-generated recommendations on a schedule — per configured agent, per
user. Requires AI features. Config under `periodic_config`:
