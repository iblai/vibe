...
`Mentor Editor` / `Mentor Viewer`, `Analytics Viewer`, `Notification Manager`,
...
![Account Management — Policies](https://raw.githubusercontent.com/iblai/vibe/refs/heads/main/skills/users/iblai-vibe-rbac/account-management-policies.png)
...
sharing (editor / chat roles on a single agent) use `<AgentAccessTab>`
from `/iblai-vibe-agent-access` instead — it wraps the underlying
...
```
GET https://api.$DOMAIN/dm/api/core/rbac/actions/definitions/
Authorization: Token <dm-token>
```

(`$DOMAIN` is `DOMAIN` from `iblai.env`, default `iblai.app`.)
...
`/iblai-vibe-agent-access` → "Roles CRUD" for the REST contract).

Full Python definitions of every default role (with their `actions` and
`data_actions` arrays) live in
[`references/default-roles.py`](https://raw.githubusercontent.com/iblai/vibe/refs/heads/main/skills/users/iblai-vibe-rbac/references/default-roles.py).
...
| `MENTOR_CREATORS` | Create new agents (`Ibl.Mentor/Mentors/action`). |
| `MENTOR_ANALYTICS_VIEWER` | View per-agent analytics; subsumes `STUDENT`. |
...
2. **Pick the resources the role applies to.** Resources are
   ...
   `/platforms/{pk}/mentors/` covers every agent; granting on
...
3. **Create the role via `<RolesTab>` (UI) or `POST rbac/roles/` (API).**
   ...
   `/iblai-vibe-agent-access` → "Roles CRUD" for the full REST contract.
...
- `/iblai-vibe-agent-access` -- agent-scoped sharing (editor / chat roles on
  a single agent) via `<AgentAccessTab>`. Documents the underlying
  REST contract (`rbac/roles/`, `rbac/policies/`, `rbac/groups/`,
  `rbac/mentor-access/`, `rbac/teams/access/`, etc.).
- `/iblai-vibe-account` -- mounts the SDK `<Account>` modal that hosts the
