...
(`iblai-edx-pro`) so users log in with an external provider instead of a local
...
No plugin settings required for a single- or multi-organization Azure AD app; client
