...
4. **Pull in any prior critique** (optional signal): When `/iblai-vibe-design critique` has already run against the same target, its priority issues are a helpful starting point for what to tackle first. Resolve the target to a file path or URL, then:
   ```bash
   slug=$(node .claude/skills/iblai-vibe-design/scripts/critique-storage.mjs slug "<resolved>")
   node .claude/skills/iblai-vibe-design/scripts/critique-storage.mjs latest "$slug"
   ```
