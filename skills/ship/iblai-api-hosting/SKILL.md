...
- **The deployed host is also the checkout redirect allowlist.** A Stripe paywall
  redirect is accepted only for a confirmed hosting alias or an attached custom
  domain of the same organization (plus localhost). A URL reconstructed from the
  project name gets rejected. See `/iblai-vibe-monetization-app-paywall`.
