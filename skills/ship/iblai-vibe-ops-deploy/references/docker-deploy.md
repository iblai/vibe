...
hardware, Cloud Run, ECS, a Kubernetes cluster, a customer's own cloud account.
...
Add the deployment origin to the organization's **allowed redirect origins**. Sign-in
leaves for the login SPA and returns to `<origin>/sso-login-complete`; an
origin the organization does not know is a login that never comes back.
