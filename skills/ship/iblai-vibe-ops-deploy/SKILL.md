...
# 2. Guard: the server needs its platform key — but only if this app reads one.
#    An app that keeps no server-side key (it calls the platform on the signed-in
#    user's own token) ships no IBLAI_API_KEY and must not be blocked here.
if grep -rq --include='*.ts' --include='*.tsx' --include='*.js' --include='*.jsx' --include='*.mjs' \
     --exclude-dir=node_modules --exclude-dir=.next --exclude-dir=out --exclude-dir=coverage \
     'IBLAI_API_KEY' . ; then
...
#    Drop EVERY .env* and add .env.production back by name: naming the ones to
#    drop ships whatever nobody thought of — '.env*.local' does not match a
#    stray '.env.local.backup', which carries the platform key into the deploy.
rm -f app.zip
zip -qr app.zip . -x 'node_modules/*' '.git/*' '.next/*' 'out/*' 'coverage/*' \
  'playwright-report/*' 'test-results/*' 'src-tauri/target/*' 'src-tauri/gen/*' \
  '.env*' 'iblai.env' 'app.zip'
zip -q app.zip .env.production
...
(Guard 2 decides for itself: it fires only when the app's own source reads
`IBLAI_API_KEY`. An app that holds no server-side key passes straight
through — never add one to satisfy it.)
...
vercel_alias, push_state, …}`), including who owns it: `username`,
`user_email` and `user_full_name`. Anything else → see the error table.
