...
the same shape used by [vibe-starter](https://github.com/iblai/vibe/tree/main/skills/start/iblai-vibe-ops-init/assets/vibe-starter)
...
[`/iblai-vibe-ops-deploy`](https://github.com/iblai/vibe/blob/main/skills/ship/iblai-vibe-ops-deploy/SKILL.md). }}
...
READY, and prints the address as `site_url` — a subdomain the platform
assigns, or the organization's own custom domain. Re-run it to redeploy;
custom domains are attached and managed through the skill, which returns
...
`pnpm exec tauri …` commands. Do NOT duplicate the full skill — keep
...
Tauri directly (`pnpm exec tauri …`) (full guide:
[`/iblai-vibe-ops-build`](https://github.com/iblai/vibe/blob/main/skills/ship/iblai-vibe-ops-build/SKILL.md)).
...
pnpm exec tauri ios init                          # one-time
xcrun simctl list devices                            # list simulators
pnpm exec tauri ios dev "iPhone 16 Pro Max"       # run on simulator
pnpm exec tauri ios build                         # produce .ipa
# CI: copy assets/tauri/workflows/tauri-build-ios.yml.j2 (see /iblai-vibe-ops-build)                 # generate App Store CI
```
...
Use the Claude Code skills to add new features:

- `/iblai-vibe-auth` — SSO authentication
- `/iblai-vibe-profile` — user profile
- `/iblai-vibe-account` — account/org settings
...
- [Vibe Starter](https://github.com/iblai/vibe/tree/main/skills/start/iblai-vibe-ops-init/assets/vibe-starter) — pre-wired Next.js + ibl.ai SSO template
...
- **Source of truth:** the bundled starter's README at
  `skills/start/iblai-vibe-ops-init/assets/vibe-starter/README.md` plus the
...
- **Do not reference internal repos.** Only the public, open-source
  ...
  bundled vibe-starter under `skills/start/iblai-vibe-ops-init/assets/`) is
...
- **Don't link to non-existent skills:** Cross-references to other
  `/iblai-vibe-*` skills are fine; cross-references to skills that don't
  exist are not.
- **Deploy to Vercel section.** Always include the **Deploy to Vercel**
  ...
  Vercel target. The deploy works for both static `output: 'export'`
  Tauri shells and server-rendered Next.js apps (the mode is detected
  from `next.config`). See
  [`/iblai-vibe-ops-deploy`](https://github.com/iblai/vibe/blob/main/skills/ship/iblai-vibe-ops-deploy/SKILL.md)
