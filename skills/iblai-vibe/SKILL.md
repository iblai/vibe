---
name: iblai-vibe
description: Start here for anything on the ibl.ai platform — the map from what a builder wants (a new app; users and admins; custom user or organization data; agents; memory; analytics; charging users; iOS, Android, macOS, Windows; anything with no component) to the skill or repo that does it. Use when the user mentions ibl.ai, iblai, or vibe, points an agent at github.com/iblai/vibe, asks where a capability lives, which skill or repo to install, or how to build an app with agents, users, organizations, memories, analytics, or billing. Frontend apps are built here (iblai/vibe); REST operation skills live in iblai/api; the reference app is iblai/os; marketing skills are in iblai/vibe-marketing.
globs:
alwaysApply: false
---
...
★ = the five platform families most apps read or write; each row names the
`iblai/api` skill that documents the same data over REST.

| You want… | Open | REST (`iblai/api`) |
|---|---|---|
| **A new app** — sign-in, chat with an agent, users/admins, profile, admin area, custom data | `/iblai-vibe-ops-init` (copies vibe-starter; asks only for org key + token) | `iblai-api-login` |
...
| **Anything with no component** | `/iblai-vibe-api` — server route + `Api-Token`, and the matching `iblai/api` skill | all 50 |
...
|---|---|---|
| [`iblai/vibe`](https://github.com/iblai/vibe) (this repo) | `/iblai-vibe-*` skills for building frontend apps — the vibe-starter template, SSO auth, chat, admin, metadata, memory, analytics, billing, deploy/build/release — on the `@iblai/iblai-js` SDK | `npx skills add iblai/vibe --all` |
| [`iblai/api`](https://github.com/iblai/api) | `iblai-api-*` skills that map every platform REST family to exact endpoints, plus a chat MCP server | `npx skills add iblai/api` |
