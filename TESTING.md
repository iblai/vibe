...
## Tier 1.6 — endpoint schema drift

`scripts/check-endpoint-schema.mjs` guards `iblai-api-*` skills against documenting an endpoint the backend renamed or removed — the "skill says a path the API no longer serves" drift that silently 404s a headless integration. Deterministic, **no LLM**. It fetches the live OpenAPI schema (`api.iblai.app/dm/api/docs/schema/`, override with `IBLAI_SCHEMA_FILE=` for offline/CI), extracts every callable `https://api.iblai.app/dm/...` URL a skill documents (those carrying a method — a bold `**VERB**`, a curl, or `-X`), and checks each path **structurally** against the schema: a schema `{param}` absorbs any skill segment (`{param}`, `$SHELL_VAR`, or example value), while literal segments (`documents`, `train`) must match — so a renamed resource fails, param spelling never false-positives. Scope (v1): endpoint **existence** (method + path), not request field names; only `/dm` REST (not `/edx` or the asgi streaming host). Known exceptions live in `scripts/endpoint-schema-allow.json`, each with a `reason` — a permanent "schema gap" (endpoint real, absent from the published schema) or a temporary "drift" marker (skill path wrong, remove once fixed).
