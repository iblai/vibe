...
> **Command reference & assets:** the full `iblai builds` subcommand surface
> (passthrough model, prereqs, `init`/`iconography`/`ci-workflow`/`device`/
> `screenshot`) is in [`references/builds-command.md`](references/builds-command.md).
> The Tauri shell templates `iblai add builds` renders are bundled in
> [`assets/tauri/`](assets/tauri/); the default app icons in
