...
2. **`.env.local`** -- write directly. Do NOT re-run any scaffolding (e.g.
   the `/iblai-auth` file generation) -- the starter already has everything
...
via the `/iblai-*` skills if the user explicitly declines the starter or
...
/iblai-ops-upgrade   # Upgrade SDK and skills to latest
/iblai-component     # Browse all available components
```

All features require auth first (`/iblai-auth`).
...
`VERCEL_TOKEN` in `iblai.env` is used by `/iblai-ops-deploy` to deploy to
...
Add features with the `/iblai-*` skills (they create the files directly).
