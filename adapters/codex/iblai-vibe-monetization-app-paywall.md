...
1. **Store the Stripe key** (skip if `GET $PAY/products/` already returns
   200): `POST https://api.$DOMAIN/dm/api/ai-account/orgs/$PLATFORM/integration-credential/`
...
4. **Create price(s)**: `POST $PAY/prices/`
...
5. **Capture display data**: `GET $PAY/prices/<id>/` → amount/currency/
