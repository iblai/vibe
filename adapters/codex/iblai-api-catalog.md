...
- **GET** `/api/catalog/enrollment/courses/completion/` - a learner's edX course and block completion, read through the Data Manager. Query params: the learner (one of `user_id`, `username`, `email`, required), `course_id` (required), `endpoint` (required, one of `status` | `outline` | `blocks` | `progress`), and `platform_key`/`org`. Returns the edX JSON verbatim: `status` = overall completion (`completed_units`, `total_units`, `completion_percentage`, `started`, `completed`), `outline` = the course block tree with per-block completion, `blocks` = per-block `completion`/`complete`/`resume_block`, `progress` = the Progress-tab payload (`completion_summary`, grades). Allowed for the learner themselves, a platform admin, and a watcher of the learner; a `403` means none of those applies (the same `403` is returned for a learner who does not exist or is not on the platform). Other statuses: `400` bad `course_id`/`endpoint`, `404` course not on the platform or the learner is not actively enrolled in it (the same `404` for both), `502` edX unreachable or errored. The platform comes from the token; naming a different `platform_key` is refused. Requires RBAC action `Ibl.Catalog/LearnerCompletion/read` on the learner (tenant admins have it; watchers get it through their watched group), or `Ibl.Catalog/LearnerCompletionSelf/read` for a member reading their own data. Use this instead of calling the LMS completion endpoints with a learner's username: edX ignores `username` for a non-staff token and silently returns the caller's own completion.
