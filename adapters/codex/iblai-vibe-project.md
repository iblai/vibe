...
![Project landing](https://raw.githubusercontent.com/iblai/vibe/refs/heads/main/skills/agents/iblai-vibe-project/iblai-vibe-project-1-landing.png)
![Project files modal](https://raw.githubusercontent.com/iblai/vibe/refs/heads/main/skills/agents/iblai-vibe-project/iblai-vibe-project-2-files-modal.png)
...
- `/iblai-vibe-agent-chat` complete (providers wrapped, store reducers
  ...
  applies.
- An agent ID — same as `/iblai-vibe-agent-chat`.
...
The project surface inherits **all** of `/iblai-vibe-agent-chat`'s setup.
Don't proceed until you confirm:

| Check | Command / file |
|---|---|
| Auth providers tree wrapped in `<ServiceWorkerProvider>` | `providers/index.tsx` |
| `skip={isSsoLoginRoute}` on `AuthProvider` + `TenantProvider` | `providers/index.tsx` |
...
```bash
pnpm add @iblai/iblai-js@^2.18.1 @iblai/agent-ai@^2.10.0
```
...
| `mentorId` from `useParams<{ mentorId; projectId }>()` | Project chats are agent-scoped — the WebSocket session still needs an agent. |
