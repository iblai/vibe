...
> Scaffold a new ibl.ai app or add ibl.ai features to an existing Next.js project — the project templates the iblai CLI renders and what its `startapp` / `add` / `config` commands do. Use when creating a new app, running `iblai startapp` or `iblai add`, wiring up the base providers/store/auth, or reproducing that scaffolding by hand. Holds the base + agent project templates as assets. For per-feature wiring see /iblai-auth; for the desktop/mobile shell see /iblai-ops-build.

# /iblai-scaffold

How an ibl.ai app gets its skeleton: the **project templates** the `iblai`
CLI renders, and the behavior of the commands that render them
(`iblai startapp`, `iblai add`, `iblai config`). When the CLI is retired,
the templates here are the record of what a generated app contains, and the
references are the record of how each command assembled it.
...
| Path | Command | Result |
|---|---|---|
| **New app** | `iblai startapp agent -o iblai-init` (then copy back) | A complete app — auth, providers, Redux store, a chat page, Tauri-ready |
| **Existing app** | `iblai add auth` (then `add profile`, `add account`, …) | ibl.ai features layered onto a vanilla Next.js project |

Both read `PLATFORM`/`TOKEN`/`DOMAIN` from `iblai.env` and derive the
`NEXT_PUBLIC_*` vars into `.env.local` (see
