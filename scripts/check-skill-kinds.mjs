...
import { CATEGORIES, listSkills } from "./lib/skills.mjs";
...
for (const { name, file } of listSkills()) {
  ...
  const core = /^metadata:\n(?:[ \t]+[^\n]*\n)*?[ \t]+core:\s*true/m.test(fm);
  if (core) coreSeen.add(name);
  if (core && !CORE.has(name)) fail("metadata.core: true but not listed under \"core\" in scripts/skill-categories.json");
  if (!core && CORE.has(name)) fail("listed as core in scripts/skill-categories.json but SKILL.md lacks metadata.core: true");
...
for (const n of CORE) if (!coreSeen.has(n) && !listSkills().some((s) => s.name === n)) { failed = true; console.error(`${n}: listed as core but no such skill`); }
if (failed) process.exit(1);
console.log(`check-skill-kinds: core=${coreSeen.size} ${Object.entries(counts).map(([k, n]) => `${k}=${n}`).join(" ")} (${Object.values(counts).reduce((a, b) => a + b, 0)} skills)`);
